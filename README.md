...
<dl> 
 <dt>Makers Coding bootcamp</dt>  
  ...
  <dd>The first independent group project which reintroduced all the recently covered topics  with the challenge of building  a web application similar to Air BnB. This got my group to think about minimum-viable-product’s (MVPs), which was important for demonstrating the core application use of the product and how it would function. For effective communication,  our team  scheduled  morning and afternoon meetings on Zoom to discussed the functions that had been implemented  and tested.  When implementing new  features the group would pair off into two pairs of two to complete the task. In addition we regularly communicated  through a Slack channel in case issues or concerns occurred with the task at hand. Furthermore, domain-driven design (DDD) was utilised to appropriately name the class names variables and methods which would match the business domain. </dd> 
